...
if(isset($_POST['login'])) {
    ...
    if(empty($_POST['email']) || empty($_POST['password'])) {
        set_flash('error', 'Email and/or Password can not be empty!', 'login.php');
    } else {
		
		$email = strip_tags($_POST['email']);
		$password = strip_tags($_POST['password']);

    	$user = $conn->query("select * from tbl_admin where email = '$email' and password = '$password'"); 
 
        if($user->num_rows == 0) {
            set_flash('error', 'Login information does not match', 'login.php');
            
        } else {       
           $fetch_user = $user->fetch_assoc();
