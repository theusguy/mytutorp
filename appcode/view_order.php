...
if (isset($_GET['id'])) {
  $updatemsgid = $_GET['id'];
  $query = "UPDATE tbl_message SET notify = '1' WHERE assignment_id = '$updatemsgid'";
  ...
  if(isset($_POST['uploadfile'])){
      ...
      redirect('view_order.php?id='.$_REQUEST['id']);
  }
  ?>
              ...
              <div class="slider-bottom-box">
                <div class="row align-items-center clearfix">
                  <div class="col-lg-4 mb-3 mb-lg-0">
                    <h2 class="mb-3 h1 t300">View Orders</h2>
                      </div>
      ...
      <div class="container">
       ...
       <div class="title">
                ...
                <?php  $payment_check =  get($conn, 'tbl_payments', "where assignment_id = '$order_id'"); ?>
                 <p><b>Status :</b> <?php if($payment_check->num_rows==0){echo 'Pending';}elseif($payment_check->num_rows==1){echo 'Completed';} ?></p>
                <?php if($payment_check->num_rows == 1){ ?>
                <p><b>Payment Method :</b> Paypal</p>
                <?php }else{ ?>
                <p><a href="<?php echo base_url(); ?>make_payment.php?ORDER_ID=<?php echo $order['id']; ?>" class="btn btn-success btn-xs">Make a Payment</a></p>
                <?php } ?>
                <div class="title">
                <h3>Project Status</h3>
             </div>
                 <?php if($order['writer_id'] != 0){ ?>
                <p><b>Writer Username :</b> <?php
                                                                 $userDetail = get($conn, 'tbl_writers', "where id = '{$order['writer_id']}'")->fetch_assoc();
                                                                echo $userDetail['username']; ?></p>


                <p><b>Delivery Date :</b> <?php echo $order['deadline']; ?></p>
                <?php } ?>
                <p><b>Status :</b> <?php if($order['status']==0){echo 'Pending';}elseif($order['status']==1){echo 'In Process';}elseif($order['status']==2){echo 'Completed';} ?></p>
          </div>
